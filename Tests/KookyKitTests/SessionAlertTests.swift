...
/// attention or a command failing fires `onSessionAlert`; same-state re-fires
/// and non-attention transitions stay quiet. Visibility gating + the actual
/// banner live in `AppDelegate` / `NotificationManager` (AppKit state, tested
/// manually).
@MainActor
final class SessionAlertTests: XCTestCase {
    ...
    func testRunningAndEndedFireNoAlert() {
        ...
        XCTAssertTrue(alerts.isEmpty, "only attention / failure should alert")
    }
